...
const requiredTables = ["workspaces", "plans", "plan_revisions", "tasks", "execution_records", "reviews", "app_sessions", "diary_entries"] as const;
const requiredRelations = ["workspaces 1:N plans", "plans 1:N plan_revisions", "plans 1:N tasks", "tasks 1:N execution_records", "plans 1:N reviews", "reviews 0:1 plans(next_plan_id)", "workspaces 1:N diary_entries"] as const;
const requiredIndexes = ["plans.workspace_id", "plan_revisions.plan_id", "tasks.plan_id", "tasks.due_date", "tasks.status", "tasks.tag", "execution_records.task_id", "reviews.plan_id", "app_sessions.user_id", "diary_entries.workspace_id"] as const;
const requiredFields: Record<string, Record<string, Record<string, unknown>>> = {
  workspaces: { id: { type: "uuid", nullable: false, primaryKey: true }, slug: { type: "text", nullable: false, unique: false, uniquePerOwner: true }, owner_id: { type: "uuid", nullable: true, references: "auth.users.id" }, title: { type: "text", nullable: false }, timezone: { type: "text", nullable: false, default: "Asia/Seoul" }, created_at: { type: "timestamptz", nullable: false } },
  ...
  app_sessions: { session_id: { type: "text", nullable: false, primaryKey: true }, user_id: { type: "uuid", nullable: false, references: "auth.users.id" }, created_at: { type: "timestamptz", nullable: false }, last_seen_at: { type: "timestamptz", nullable: false }, revoked_at: { type: "timestamptz", nullable: true } },
  diary_entries: { id: { type: "uuid", nullable: false, primaryKey: true }, workspace_id: { type: "uuid", nullable: false, references: "workspaces.id" }, record_date: { type: "date", nullable: false }, question: { type: "text", nullable: false }, metric_name: { type: "text", nullable: false }, unit: { type: "text", nullable: false }, value: { type: "numeric", nullable: false }, calculation_rule: { type: "text", nullable: false }, plan_rule_version: { type: "integer", nullable: false, minimum: 1 }, plan_rule: { type: "text", nullable: false }, rule_change_reason: { type: "text", nullable: true }, entry_origin: { type: "text", nullable: false, values: ["user_entered", "synthetic_test"] }, created_at: { type: "timestamptz", nullable: false }, updated_at: { type: "timestamptz", nullable: false } },
};
...
export function verifySeedSafety(seed: string): void {
  ...
  if (!/\binsert\s+into\s+public\.workspaces\b/.test(sql) || !/'public'/.test(sql)) throw new Error("Seed must target the public workspace.");
}
