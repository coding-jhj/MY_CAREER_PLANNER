...
function errorResponse(error: unknown): Response {
  ...
  if (error instanceof UnauthorizedError) return Response.json({ error: "Authentication required" }, { status: 401 });
...
export async function GET(): Promise<Response> {
  try {
    ...
    const exported = await getExportData(
      ...
      new Date().toISOString(), new SupabaseDiaryRepository(client),
    );
